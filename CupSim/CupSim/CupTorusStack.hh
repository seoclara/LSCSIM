...
// $Id: CupTorusStack.hh,v 1.1.1.1 2016/10/31 08:41:44 ejjeon Exp $
//
...
//
// CupTorusStack(const G4String      &pName )
//
...
//
//
// 1999.11.22 G.Horton-Smith First version of CupTorusStack

#ifndef CupTorusStack_HH
#define CupTorusStack_HH

#include "G4CSGSolid.hh"
#include "G4Version.hh"

class CupTorusStack : public G4CSGSolid {
  public:
    CupTorusStack(const G4String &pName);

    virtual ~CupTorusStack();
    ...
    void SetAllParameters(G4int n,                   // number of Z-segments
                          ...
                          CupTorusStack *inner = 0); // inner TorusStack (opt.)
    ...
    G4double DistanceToOut(const G4ThreeVector &p, const G4ThreeVector &v,
                           const G4bool calcNorm = G4bool(false), G4bool *validNorm = 0,
                           G4ThreeVector *n = 0) const;
    G4double DistanceToOut(const G4ThreeVector &p) const;

    // Naming method (pseudo-RTTI : run-time type identification)
    virtual G4GeometryType GetEntityType() const { return G4String("CupTorusStack"); }

    // Visualisation functions
    void DescribeYourselfTo(G4VGraphicsScene &scene) const;
    G4VisExtent GetExtent() const;
    G4Polyhedron *CreatePolyhedron() const;
    // Get point functions for overlap checking
  ...
  protected:
    ...
    double *area_ratio_segment;
    double *z_dist_segment;
    double *phi_start_segment;
    double *phi_end_segment;
    double max_rho;        // maxium distance of surface from Z axis
    double myRadTolerance; // because Geant4.1.0 default is too small
    CupTorusStack *inner;  // because G4SubtractionSolid is bad
};

...
#include "CupSim/CupPosGen.hh" // for Strip() utility function
...
////////////////////////////////////////////////////////////////
const char *CupVertexGen_Gun::theElementNames[] = {
    ...
    "Bk", "Cf", "Es", "Fm", "Md", "No", "Lr", "Rf", "Db", "Sg", "Bh", "Hs", "Mt", "Xa"

};

CupVertexGen_Gun::CupVertexGen_Gun(const char *arg_dbname)
    : CupVVertexGen(arg_dbname), _mom(0., 0., 0.), _ke(0.0), _pol(0., 0., 0.), _multiplicity(1) {
    _pDef = G4ParticleTable::GetParticleTable()->FindParticle("geantino");
}

CupVertexGen_Gun::~CupVertexGen_Gun() {}
...
void CupVertexGen_Gun::GeneratePrimaryVertex(G4Event *argEvent) {
...
/** Set state of the CupVertexGen_Gun, or show current state and
  explanation of state syntax if empty string provided.

  Format of argument to CupVertexGen_Gun::SetState:
  ...
  */
void CupVertexGen_Gun::SetState(G4String newValues) {
    CupVPosGen::Strip(newValues);
    if (newValues.length() == 0) {
        // print help and current state
        G4cout << "Current state of this CupVertexGen_Gun:\n"
               << " \"" << GetState() << "\"\n"
               << G4endl;
        G4cout << "Format of argument to CupVertexGen_Gun::SetState: \n"
                  " \"pname  momx_MeV momy_MeV momz_MeV  KE_MeV  polx poly polz mult\"\n"
    ...
    if (newTestGunG4Code == NULL) {
        ...
        // EJ: set E for metastable state
        if (pname[pname.length() - 1] == 'm') {
            is >> E;
        } else
            E = 0.0;
        // G4cout << "EJ: E= " << E << G4endl;
        ...
        if (A > 0) {
            ...
            if (Z <= numberOfElements)
                ...
                newTestGunG4Code = G4ParticleTable::GetParticleTable()->GetIonTable()->GetIon(
                    Z, A, E); // EJ: for excitation level [keV]
#else
                newTestGunG4Code = G4ParticleTable::GetParticleTable()->GetIon(Z, A, E); // EJ: for
#endif
        }
        if (newTestGunG4Code == NULL) {
...
G4String CupVertexGen_Gun::GetState() {
    std::ostringstream os;

    if (_pDef == 0) {
        _pDef = G4ParticleTable::GetParticleTable()->FindParticle("geantino");
    }
    os << _pDef->GetParticleName() << '\t' << _mom.x() << ' ' << _mom.y() << ' ' << _mom.z() << '\t'
       << _ke << '\t' << _pol.x() << ' ' << _pol.y() << ' ' << _pol.z() << '\t' << _multiplicity
       << std::ends;
    G4String rv(os.str());
...
CupVertexGen_HEPEvt::CupVertexGen_HEPEvt(const char *arg_dbname) : CupVVertexGen(arg_dbname) {
    _filename = "";
    _file     = 0;
    _isPipe   = false;
}

CupVertexGen_HEPEvt::~CupVertexGen_HEPEvt() { Close(); }

void CupVertexGen_HEPEvt::Open(const char *argFilename) {
    if (argFilename == NULL || argFilename[0] == '\0') {
        G4cerr << "CupSim/CupVertexGen_HEPEvt::Open(): null filename" << G4endl;
        return;
    }
    if (_file != 0) Close();
...
void CupVertexGen_HEPEvt::Close() {
    if (_file) {
        if (_isPipe) {
            int status = pclose(_file);
            if (status != 0) {
                G4cerr << "HEPEvt input pipe from " << _filename << " gave status " << status
                       << " on close." << G4endl;
            }
            _file     = 0;
            _isPipe   = false;
...
void CupVertexGen_HEPEvt::GetDataLine(char *buffer, size_t size) {
    int rewind_count = 0;
    char firstword[64];
    ...
    for (;;) {
        ...
        if (fgets(buffer, size, _file) != buffer) {
            ...
            G4cerr << (feof(_file) ? "End-of_file reached" : "Failure") << " on " << _filename
                   << " at " << ftell(_file);
            if (rewind_count == 0) {
                if (_isPipe == false) {
        ...
        if (strcmp(firstword, "STATE") == 0) {
            ...
            while (fgets(buffer, size, _file) == buffer) {
                ...
                if (nscan == 2)
                    db[((_dbname + ".") + firstword).c_str()] = value;
                else {
...
/** Generates one or more particles with type, momentum, and
  ...
  */
void CupVertexGen_HEPEvt::GeneratePrimaryVertex(G4Event *argEvent) {
    // this is a modified and adapted version of G4HEPEvt
    // (which itself may be a modified and adapted version of CLHEP/StdHep++...)

    if (_file == 0) {
        G4cerr << "CupSim/CupVertexGen_HEPEvt::GeneratePrimaryVertex: "
                  "Error, no file open!"
               << G4endl;
        return;
    }
    ...
    for (int IHEP = 0; IHEP < NHEP; IHEP++) {
        ...
        G4double req_vertex_dT = req_novalue; // vertex _delta_ time (in ns, a CupHEPEvt convention)
        ...
        GetDataLine(buffer, sizeof(buffer));
        if (_file == 0) {
            G4cerr << "Unexpected end of file in " << _filename << ", expecting particle " << IHEP
                   << "/" << NHEP << G4endl;
            Close();
            return;
        }
        istringstream is(buffer);
        is >> ISTHEP >> IDHEP >> JDAHEP1 >> JDAHEP2 >> PHEP1 >> PHEP2 >> PHEP3 >> PHEP5 >>
        ...
        if (ISTHEP != kISTHEP_ParticleForTracking) {
            if (ISTHEP <= 0 || ISTHEP > kISTHEP_Max) {
                G4cerr << "Error in CupVertexGen_HEPEvt[" << _filename << "]: ISTHEP=" << ISTHEP
...
#else
            G4ParticleDefinition *g4code = G4ParticleTable::GetParticleTable()->GetIon(Z, A, 0.0);
#endif
            if (g4code == 0) {
                G4cerr << "Warning: CupHEPEvt could not find Ion Z=" << Z << " A=" << A
                       << " code=" << IDHEP << G4endl;
                particle = new G4PrimaryParticle(IDHEP, PHEP1 * energy_unit, PHEP2 * energy_unit,
                                                 PHEP3 * energy_unit);
            } else {
                particle = new G4PrimaryParticle(g4code, PHEP1 * energy_unit, PHEP2 * energy_unit,
                                                 PHEP3 * energy_unit);
            }
        } else if (IDHEP == 0 && fabs(PHEP1) + fabs(PHEP2) + fabs(PHEP3) < 20e-9) {
            // special case: E less than ~10 eV and pdgcode=0 means optical photon
            particle = new G4PrimaryParticle(G4OpticalPhoton::OpticalPhoton(), PHEP1 * energy_unit,
        ...
        if (req_vertexX == req_novalue && req_vertexY == req_novalue &&
            ...
            for (iv = vertexSet.size() - 1; iv >= 0; iv--) {
                if (vertexSet[iv]->GetT0() == vertexT && vertexSet[iv]->GetPosition() == position)
                    break; // found it
            }
            if (iv >= 0) // found?
            ...
            for (int j = jda1; j <= jda2; j++) {
                ...
                if (HPlist[j]->GetISTHEP() > 0) {
                    if (vertexList[i] == vertexList[j]) {
                        ...
                        G4cerr << "Error in  CupVertexGen_HEPEvt[" << _filename << "]: mother " << i
                               << " and daughter " << j
...
/** Set source of HEPEVT ascii data for this generator, or print
  ...
  */
void CupVertexGen_HEPEvt::SetState(G4String newValues) {
    CupVPosGen::Strip(newValues);
    if (newValues.length() == 0) {
        // print help and current state
        G4cout << "Current state of this CupVertexGen_HEPEvt:\n"
               << " \"" << GetState() << "\"\n"
               << G4endl;
        G4cout << "Format of argument to CupVertexGen_HEPEvt::SetState: \n"
...
G4String CupVertexGen_HEPEvt::GetState() { return G4String(_filename); }

////////////////////////////////////////////////////////////////

CupVertexGen_Stack::CupVertexGen_Stack(const char *arg_dbname, CupPrimaryGeneratorAction *argCupPGA)
    : CupVVertexGen(arg_dbname) {
    fCupPGA = argCupPGA;
}

CupVertexGen_Stack::~CupVertexGen_Stack() {}

void CupVertexGen_Stack::GeneratePrimaryVertex(G4Event *argEvent) {
    // pop and add all vertices within time window
    G4double eventEndTime = fCupPGA->GetUniversalTime() + fCupPGA->GetEventWindow();

    while (!_stack.empty()) {
        _stack_iterator_t b = _stack.begin();
        G4PrimaryVertex *v  = *b;
        if (v->GetT0() > eventEndTime) break;
        v->SetT0(v->GetT0() - fCupPGA->GetUniversalTime());
	if(v->GetT0()<0) v->SetT0(0); //EJ: for the correction of a negative global time
    ...
    if (!_stack.empty()) {
        ...
        fCupPGA->NotifyTimeToNextStackedEvent(nextTime - fCupPGA->GetUniversalTime());
    }
}

void CupVertexGen_Stack::StackIt(const G4Track *track) {
    G4PrimaryVertex *vertex = new G4PrimaryVertex(
        track->GetPosition(), track->GetGlobalTime() + fCupPGA->GetUniversalTime());
...
void CupVertexGen_Stack::SetState(G4String /*newValues*/) {
    G4cout << "CupSim/CupVertexGen_Stack has no user-settable state." << G4endl;
...
G4String CupVertexGen_Stack::GetState() {
    G4String rv("");
    return rv;
}
...
#if 0
/*  Below are the standard components for a "CupSim/CupVertexGen": */


CupVertexGen_XXX::CupVertexGen_XXX(const char *arg_dbname)
  : CupVVertexGen(arg_dbname)
{
  ...
}

CupVertexGen_XXX::~CupVertexGen_XXX()
{
  ...
}

void CupVertexGen_XXX::
...
SetState(G4String newValues)
{
  ...
  if (newValues.length() == 0) {
    // print help and current state
    G4cout << "Current state of this CupVertexGen_XXX:\n"
      << " \"" << GetState() << "\"\n" << G4endl;
    G4cout << "Format of argument to CupVertexGen_XXX::SetState: \n"
      << G4endl;
    return;
  }
  ...
  ...
    ...
    std::istrstream is(newValues.c_str());
}


G4String CupVertexGen_XXX::
GetState()
{
  std::ostrstream os;

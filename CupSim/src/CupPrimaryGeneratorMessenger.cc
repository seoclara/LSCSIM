...
#include "CupSim/CupPosGen.hh"    // for GetState(), SetState()
#include "CupSim/CupVertexGen.hh" // for GetState(), SetState()

CupPrimaryGeneratorMessenger::CupPrimaryGeneratorMessenger(CupPrimaryGeneratorAction *myaction)
    : myAction(myaction) {
    ...
    PileupCmd = new G4UIcommand("/generator/disablePileup", this);
    PileupCmd->SetGuidance("Set/show disabling pileup event generation");
    PileupCmd->SetParameter(new G4UIparameter("disable", 'b', true));
}

CupPrimaryGeneratorMessenger::~CupPrimaryGeneratorMessenger() {
    delete ListCmd;
    delete RateCmd;
    delete GunCmd;
    delete VtxSetCmd;
...
void CupPrimaryGeneratorMessenger::SetNewValue(G4UIcommand *command, G4String newValues) {
    if (command == ListCmd) {
        G4cout << "Generator codes and descriptions:\n"
               << " evt  pos  vtx  name (== posname:vtxname)\n";
        for (int i = 0; i < myAction->theNumEventTypes; i++) {
            G4cout << std::setw(4) << i << " " << std::setw(4)
                   << myAction->GetPositionCodeForEventType(i) << " " << std::setw(4)
        ...
        if (strlen(id) == 0) {
            ...
            G4cout << "Rates:\n";
            for (int i = 0; i < myAction->theNumEventTypes; i++) {
                G4cout << std::setw(3) << i << " " << std::setw(7)
            ...
            if (index < 0 || index >= myAction->theNumEventTypes) {
                G4cout << "Index must be non-negative and less than " << myAction->theNumEventTypes
                       << ", or must be a name exactly matching a defined generator."
                       << "  (No match for \"" << id << "\" was found.)" << G4endl;
            } else {
                is >> rate;
                if (is.fail()) {
                    G4cout << std::setw(3) << index << " " << std::setw(7)
        ...
        if (newValues.length() > 0) do { // this "do" is just to allow "break" to exit this block
                // get UI pointer for ApplyCommand
                G4UImanager *theUI = G4UImanager::GetUIpointer();
                // parse and set new values
                std::istringstream is((const char *)newValues);
                std::ostringstream os_vtx;
                std::ostringstream os_pos;
                ...
                os_pos << "/generator/pos/set " << myAction->kGunPosIndex << " \"" << x << ' ' << y
                       << ' ' << z << "\"" << std::ends;
        ...
        if (index < 0 || index >= myAction->theNumVertexGenCodes) {
            G4cerr << "/generator/vtx/set: invalid index or name: arguments are \"" << newValues
                   << "\"" << G4endl;
            return;
        }
        ...
        if (index < 0 || index >= myAction->theNumPosGenCodes) {
            G4cerr << "/generator/pos/set: invalid index or name: arguments are \"" << newValues
                   << "\"" << G4endl;
            return;
        }
        ...
        if (newValues.length() == 0) {
            G4cout << "Current window is " << myAction->GetEventWindow() / ns << " ns" << G4endl;
        } else {
            std::istringstream is((const char *)newValues);
        ...
        if (newValues.length() == 0) {
            G4cout << "Current clip time is " << myAction->GetChainClip() / ns << " ns" << G4endl;
        } else {
            std::istringstream is((const char *)newValues);
            G4double newWindow = -1.0;
            is >> newWindow;
    ...
    } else if (command == PileupCmd) {
        myAction->SetPileupStatus(StoB(newValues));
    } else {
        G4cerr << "invalid Cup \"set\" command";
    }
}

G4String CupPrimaryGeneratorMessenger::GetCurrentValue(G4UIcommand *command) {
    if (command == RateCmd) {
        std::ostringstream os;
        os << "Rates:\n";
        for (int i = 0; i < myAction->theNumEventTypes; i++) {
            os << std::setw(3) << i << " " << std::setw(7)
               << myAction->GetEventRate(i) / (1. / second)
               << (myAction->GetEventTriggerCondition(i) ? " * " : "   ")
               << myAction->GetEventTypeName(i) << G4endl;
        }
        ...
        G4String vstring = myAction->GetVertexGenerator(myAction->kGunVtxIndex)->GetState();
        G4String pstring = myAction->GetPositionGenerator(myAction->kGunPosIndex)->GetState();
        ...
        std::ostringstream os;
        os << myAction->GetEventWindow() << std::ends;
        G4String rv(os.str());
        //      os.freeze(0);
        return rv;
    } else if (command == ChainClipCmd) {
        std::ostringstream os;
        os << myAction->GetChainClip() << std::ends;
        G4String rv(os.str());
        //      os.freeze(0);
        return rv;
    } else if (command == PileupCmd) {
        return BtoS(myAction->GetPileupStatus());
    }

    return G4String("invalid CupPrimaryGeneratorMessenger \"get\" command");
}

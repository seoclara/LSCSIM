...
////////////////////////////////////////////////////////////////
// Make the PMTs for the buffer tank
void LscDetectorConstruction::ConstructLscYemilab_ID() {
	...
	G4VPhysicalVolume *BufferInteriorPhys = GetPhysicalVolumeByName("BufferInteriorPhys");
	...
	if (whereInnerPMT.fail()) {
		...
		if (getenv("LscDATA") == NULL) {
			G4cerr << 
					"LscDATA environment variable is not set, so I was looking for"
					<< basic_fn << " in the current directory." 
			<< G4endl;
		} else {
			G4cerr << 
					"I was looking for it in the LscDATA directory, "
					<< getenv("LscDATA") 
			<< G4endl;
		}
	...
	G4cout << "maxIDPMTNo = " << maxIDPMTNo << G4endl;

	///////////////////////////////////////////////////////////////////////////
	// --- PMT sensitive detector
	///////////////////////////////////////////////////////////////////////////
	G4SDManager *fSDman = G4SDManager::GetSDMpointer();
	CupPMTSD *pmtSDInner = new CupPMTSD("/cupdet/pmt/inner", maxIDPMTNo, 0, 20);
	...
	while (whereInnerPMT.good()) {
		...
		//double normal_angle = (cood_z > 0.0 ? -M_PI / 2 : M_PI / 2);  // JW: comment out (2024.02.13.)
